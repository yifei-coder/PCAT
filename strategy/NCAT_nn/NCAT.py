...
class env:
    def __init__(self,data,concept_map,config,T, pth_path = None):
        self.config = config
        self.T = T
        self.CDM = config['cdm']
        self.rates = {}
        ...
        if pth_path == None:
            pth_path=f'../model/ckpt/{self.CDM}.pt'
    ...
    def step(self, action, sid):
        ...
        info = {"ACC": acc,
                "AUC": auc,
                "rate":rate}
        self.state[1].append([action, reward, done, info])  #状态转移
        return self.state, reward, done, info

    def reward(self, action,sid, config = None, premodel=None, concept_map=None, metadata=None):
        ...
        correct = []
        for it in items:
            if it in self.rates[sid]:   #这边sid与self.state[0][0]等同
                correct.append(self.rates[sid][it])
            else:
                if(premodel == None):
                    print('当前学生:', sid)
                    print('当前题目:',it)
                    print("reward中用于预测的模型未加载")

                stu = torch.tensor(sid).to(config['device'])
                concepts = concept_map[int(it)]
                prob = torch.tensor(int(it)).to(config['device'])
                concepts_emb = [0.] * metadata['know_num']
                for concept in concepts: 
                    concepts_emb[concept] = 1.0 
                concepts_emb = torch.tensor(concepts_emb).to(config['device'])
                #print(f"sid: {sid}, type: {type(sid)}")
                #print(f"it: {int(it)}, type: {type(int(it))}")
                output = premodel.model(stu, prob, concepts_emb)
                #print(f"Model output shape: {output.shape}")
                precorrect = 1 if np.random.random() < output.view(-1)[0].item() else 0
                #print('转换之后:',precorrect)
                correct.append(precorrect)
                add_prob[it] = precorrect
                #print('一次预测结束')
        #-----------------------------------------------------------
        ...
        #原数据里面加入这个答题记录
        if add_prob != {}:
            for it in add_prob.keys():
                self.tsdata.data[sid][it] = add_prob[it]
...
class NCATModel():
    ...
    def __init__(self, NCATdata,concept_map,config,test_length, path):
        super().__init__()
        self.config = config
        self.model = None
        self.env = env(data=NCATdata,concept_map=concept_map,config=config,T=test_length, pth_path = path)
        self.memory = []
    ...
    def ncat_policy(self,sid,THRESHOLD,used_actions,type,epoch):
        ...
        while not done:
            ...
            for item in actions: policy[item] = -np.inf #已经选择过得题目位置赋值为负无穷，代表下次不会再被选择
            if type == "training":  #这句代码是后加的
                for item in range(self.item_num):
                    if item not in self.env.candidate_items:
                        policy[item] = -np.inf
            policy[-1] = -np.inf    #将额外索引（比如题目数量为454，0-453，就将最后一个即454赋值为负无穷）

            action = np.argmax(policy[1:]) + 1  #得到最大值的题目对应的索引
