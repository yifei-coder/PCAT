...
def get_top_k_concepts(datatype: str, topk: int = 10): 
    ...
    all_list = list(sorted_dict.keys()) 
    return all_list[:topk]  


def get_average_correct_rate(datatype: str):  
    data = pd.read_csv('../data/{}/TotalData.csv'.format(datatype), header=None).to_numpy() 
    right = 0   
...
def get_group_by_correct_rate(datatype: str):  
    ...
    with open('../data/{}/high.pkl'.format(datatype), 'wb') as f:
        pickle.dump(high_indices, f) 
    with open('../data/{}/middle.pkl'.format(datatype), 'wb') as f:
        pickle.dump(middle_indices, f) 
    with open('../data/{}/low.pkl'.format(datatype), 'wb') as f:
