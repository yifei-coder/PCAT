...
class IRTModel(AbstractModel): 
    ...
    def train(self, train_data: TrainDataset, log_step=1, out=False): 
    ...
    def adaptest_save(self, path): 
        ...
        model_dict = {k:v for k,v in model_dict.items() if 'alpha' in k or 'beta' in k or 'theta' in k} 
        torch.save(model_dict, path) 

    def adaptest_load(self, state_path): 
        """
        ...
        #如果有GPU
        #self.model.load_state_dict(torch.load(state_path), strict=False) 
        #如果没有GPU
        self.model.load_state_dict(torch.load(state_path, map_location=self.config['device']), strict=False)
        self.model.to(self.config['device']) 

    def adaptest_update(self, adaptest_data: AdapTestDataset,sid=None, last=False): 
        """
